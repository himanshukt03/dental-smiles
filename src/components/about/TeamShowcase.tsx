...
export default function TeamShowcase({ doctors, staff }: TeamShowcaseProps) {
  ...
  // Extract a brief 1-2 sentence preview from the full bio
  const getBioPreview = (bioText: string) => {
  ...
  return (
    <>
      {/* Doctors Section */}
      <section className="py-12 md:py-16" style={{ backgroundColor: '#741234' }}>
        <div className="container-clinical">
          <div className="text-center mb-12 max-w-3xl mx-auto space-y-3">
            <span className="inline-flex items-center gap-1.5 rounded-full bg-white/10 px-3.5 py-1 text-xs font-semibold uppercase tracking-widest text-white/90 border border-white/15">
              <Award className="h-3.5 w-3.5" /> Experienced Clinicians
            </span>
            ...
            {doctors.map((doctor) => {
              ...
              return (
                <div
                  ...
                  className="group relative overflow-hidden rounded-2xl border border-white/20 bg-white p-5 sm:p-6 shadow-md transition-all duration-300 hover:-translate-y-1 hover:shadow-2xl hover:border-white focus:outline-none focus:ring-2 focus:ring-white flex flex-col justify-between cursor-pointer text-left"
                >
                  <div className="space-y-4">
                    {/* Doctor Photo */}
                    <div className="relative aspect-[4/3] w-full overflow-hidden rounded-xl bg-slate-100 border border-primary/10">
                      <Image
                        src={imageSrc}
                        alt={doctor.name}
                        fill
                      ...
                      />
                      <div className="absolute top-3 right-3 rounded-full bg-white/95 backdrop-blur-md px-2.5 py-0.5 text-[11px] font-bold text-primary shadow-sm border border-primary/10">
                  ...
                  <div className="pt-4 mt-4 border-t border-primary/10 flex items-center justify-between">
                    ...
                    <span className="text-[11px] font-medium text-muted-foreground bg-clinical-creme px-2 py-0.5 rounded-md">
                      Click to expand
                    </span>
                  </div>
                </div>
          ...
          <div className="text-center mt-20 mb-12 max-w-3xl mx-auto space-y-3">
            <span className="inline-flex items-center gap-1.5 rounded-full bg-white/10 px-3.5 py-1 text-xs font-semibold uppercase tracking-widest text-white/90 border border-white/15">
              <Users className="h-3.5 w-3.5" /> Caring Support Staff
            </span>
            <h2 className="text-2xl sm:text-3xl md:text-4xl font-heading font-bold text-white tracking-tight">
              Meet Our Team
            </h2>
            ...
            {staff.map((member) => {
              ...
              return (
                <div
                  ...
                  className="group relative overflow-hidden rounded-2xl border border-white/20 bg-white p-4 sm:p-5 shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-xl hover:border-white focus:outline-none focus:ring-2 focus:ring-white flex flex-col justify-between cursor-pointer text-left"
                >
                  <div className="space-y-3.5">
                    ...
                    <div className="relative aspect-[4/3] w-full overflow-hidden rounded-xl bg-slate-100 border border-primary/10">
                      ...
                      />
                      {member.credentials && (
                        <div className="absolute top-2.5 right-2.5 rounded-full bg-white/95 backdrop-blur-md px-2 py-0.5 text-[10px] font-bold text-primary shadow-sm border border-primary/10">
                          {member.credentials}
                        </div>
                      )}
                    </div>
                    ...
                    <div className="flex flex-wrap gap-1 pt-0.5">
                      {member.specialties.slice(0, 2).map((specialty) => (
                        <span
                          key={specialty}
                          className="inline-flex items-center gap-1 rounded-full bg-clinical-creme px-2 py-0.5 text-[10px] font-medium text-foreground/80 border border-primary/10"
                        >
        ...
        {selectedMember && (
          <DialogContent className="max-w-2xl p-0 overflow-hidden rounded-2xl bg-white border border-primary/20 shadow-2xl">
            <div className="relative">
              ...
              <div className="bg-gradient-to-br from-primary/10 via-clinical-creme to-white p-6 sm:p-8 border-b border-primary/10">
                <div className="flex flex-col sm:flex-row items-center sm:items-start gap-5">
                  <div className="relative w-28 h-28 sm:w-32 sm:h-32 shrink-0 rounded-2xl overflow-hidden border-2 border-white shadow-lg ring-2 ring-primary/20">
                    <Image
                  ...
                  <div className="space-y-1.5 text-center sm:text-left flex-1">
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                      ...
                      {selectedMember.credentials && (
                        <span className="self-center sm:self-start inline-flex items-center rounded-full bg-primary/10 px-3 py-0.5 text-xs font-bold text-primary border border-primary/15">
                          {selectedMember.credentials}
                        </span>
                      )}
                    </div>
                    ...
                    <div className="flex flex-wrap justify-center sm:justify-start gap-1.5 pt-2">
                      {selectedMember.specialties.map((specialty) => (
                        <span
                          key={specialty}
                          className="inline-flex items-center gap-1 rounded-full bg-white/90 px-2.5 py-0.5 text-xs font-medium text-foreground border border-primary/15 shadow-sm"
                        >
